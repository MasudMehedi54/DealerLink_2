...
2. **Geocode the location on registration** (`RegisterScreen`) – call
   `https://geocoding-api.open-meteo.com/v1/search?name=<city>&count=1`, parse with Jackson like
   `WeatherResponse`, and save real lat/lon so the weather check works for new users.
3. **Keep order and delivery status in sync** (`DealerDashboard` orders tab) – setting delivery to
   `DELIVERED` should also set the order to `DELIVERED` (one DAO method in a transaction).
4. **Wrap multi-step writes in a transaction** (`ShopDashboard` accept button: accept quote +
   reject others + update request + place order) so a failure can't leave half-updated data.
5. **Move DB calls off the UI thread** – `quotesCol`/`yourBidCol` run one SQL query per row while
   rendering; load counts in one `GROUP BY` query inside a JavaFX `Task`.
6. **Dashboard KPI cards** – a row above the tabs (e.g. "Open requests 3 · Best saving ৳25 ·
   Low-stock items 1"); the CSS tokens make these easy to style.
7. **Import from JSON** – a dealer "Import stock (.json)" button re-using `JsonDataService.mapper()`
   to read a list of `InventorySeed`.
8. **Split big UI classes** – `ShopDashboard`/`DealerDashboard` build every tab inline; one class per
   tab (or FXML + controller) makes them easier to test and change.
9. **Add `module-info.java`** if you package with `jlink`/`jpackage`
